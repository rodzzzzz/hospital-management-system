...
<body class="bg-gray-50">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Consultation Queue</h3>
                        <ul class="space-y-3">
                            <li class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                <span class="font-medium text-gray-800">David Chen</span>
                                <span class="text-sm text-gray-500">Dr. Jacobs</span>
                            </li>
                            <li class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                <span class="font-medium text-gray-800">Emily White</span>
                                <span class="text-sm text-gray-500">Dr. Carter</span>
                            </li>
                            <li class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                <span class="font-medium text-gray-800">Michael Brown</span>
                                <span class="text-sm text-gray-500">Dr. Lee</span>
                            </li>
                        </ul>
                    </div>
    ...
    <script>
        ...
        async function loadOpdAppointments(dateStr) {
            ...
            const res = await fetch('api/opd/list.php?date=' + encodeURIComponent(d), { headers: { 'Accept': 'application/json' } });
        ...
        async function loadOpdAppointmentsAll() {
            ...
            const res = await fetch('api/opd/list.php?all=1', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
                opdAppointments = [];
                tbodies.forEach(tbody => {
        ...
        async function loadOpdAssessmentsAll(q) {
            ...
            try {
                ...
                const url = 'api/opd_assessment/list.php?all=1' + (qs ? ('&q=' + encodeURIComponent(qs)) : '');
        ...
        async function loadOpdAppointmentRequests() {
            ...
            const res = await fetch('api/opd/list.php?status=requested', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
        ...
        async function renderOpdAssessPatientOptions() {
            ...
            resultsEl.querySelectorAll('button[data-id]').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    try {
                        const aRes = await fetch('api/opd/list.php?patient_id=' + encodeURIComponent(String(pid)) + '&latest=1', { headers: { 'Accept': 'application/json' } });
        ...
        async function loadOpdConsultAppointments() {
            ...
            try {
                const [todayRes, inConsultationRes, completedRes] = await Promise.all([
                    fetch('api/opd/list.php?date=' + encodeURIComponent(d), { headers: { 'Accept': 'application/json' } }),
                    fetch('api/opd/list.php?all=1&status=in_consultation', { headers: { 'Accept': 'application/json' } }),
                    fetch('api/opd/list.php?all=1&status=completed', { headers: { 'Accept': 'application/json' } }),
                ]);
        ...
        async function loadOpdConsultNotesForModal(appointmentId, q) {
            ...
            try {
                ...
                const url = hasAppt
                    ? ('api/opd_notes/list.php?appointment_id=' + encodeURIComponent(String(appointmentId)) + (qs ? ('&q=' + encodeURIComponent(qs)) : ''))
                    : ('api/opd_notes/list.php?all=1' + (qs ? ('&q=' + encodeURIComponent(qs)) : ''));
                const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
                const json = await res.json().catch(() => null);
        ...
        async function submitOpdConsultNote() {
            ...
            const url = isEditing ? 'api/opd_notes/update.php' : 'api/opd_notes/create.php';
        ...
        async function loadOpdBillingForAppointment(appointmentId) {
            ...
            if (!appt) {
                const resAppt = await fetch('api/opd/get.php?id=' + encodeURIComponent(String(appointmentId)), { headers: { 'Accept': 'application/json' } });
            ...
            try {
                const [itemsRes, cashierInvRes, chargesRes] = await Promise.all([
                    fetch('api/opd_billing/list.php?appointment_id=' + encodeURIComponent(String(appointmentId)), { headers: { 'Accept': 'application/json' } }),
                    fetch('api/cashier/get_invoice_by_source.php?source_module=' + encodeURIComponent('opd_billing_summary') + '&source_id=' + encodeURIComponent(String(appointmentId)), { headers: { 'Accept': 'application/json' } }),
                    (patientCode ? fetch('api/cashier/list_charges.php?group=patient&status=pending_invoice&q=' + encodeURIComponent(patientCode), { headers: { 'Accept': 'application/json' } }) : Promise.resolve(null)),
                ]);
        ...
        async function submitOpdBillToCashier(appointmentId) {
            ...
            const res = await fetch('api/opd_billing/submit_to_cashier.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        ...
        async function submitOpdBillingItem() {
            ...
            const res = await fetch('api/opd_billing/create.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                body: JSON.stringify({
        ...
        async function deleteOpdBillingItem(itemId, appointmentId) {
            ...
            if (!Number.isFinite(id) || id <= 0) return;
            const res = await fetch('api/opd_billing/delete.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        ...
        if (opdAssessDockEditBtn) {
            opdAssessDockEditBtn.addEventListener('click', async () => {
                ...
                try {
                    const res = await fetch('api/opd_assessment/list.php?appointment_id=' + encodeURIComponent(String(opdSelectedAssessmentContext.appointment_id || '')), { headers: { 'Accept': 'application/json' } });
                    const json = await res.json().catch(() => null);
                    if (!res.ok || !json || !json.ok) throw new Error('Failed');
                    const rows = Array.isArray(json.assessments) ? json.assessments : [];
        ...
        async function loadOpdAssessments(appointmentId) {
            ...
            try {
                const res = await fetch('api/opd_assessment/list.php?appointment_id=' + encodeURIComponent(String(appointmentId)), { headers: { 'Accept': 'application/json' } });
                const json = await res.json().catch(() => null);
                if (!res.ok || !json || !json.ok) throw new Error('Failed');
        ...
        async function createOpdAppointmentForAssessment(patientId) {
            ...
            if (!Number.isFinite(pid) || pid <= 0) return null;
            try {
                const res = await fetch('api/opd/create.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                    body: JSON.stringify({
                        patient_id: pid,
        ...
        async function submitOpdAssessment() {
            ...
            const url = isEditing ? 'api/opd_assessment/update.php' : 'api/opd_assessment/create.php';
        ...
        async function viewAppointmentDetails(appointmentId) {
            ...
            if (!appointment) {
                const res = await fetch('api/opd/get.php?id=' + encodeURIComponent(String(appointmentId)), { headers: { 'Accept': 'application/json' } });
                const json = await res.json().catch(() => null);
        ...
        async function loadOpdConsultPatientDetails(patientId) {
            ...
            try {
                const res = await fetch('api/patients/list.php?id=' + encodeURIComponent(String(id)), { headers: { 'Accept': 'application/json' } });
                const json = await res.json().catch(() => null);
        ...
        async function loadOpdLabRequests() {
            ...
            const res = await fetch('api/lab/list_requests.php?mode=opd', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
                tbody.innerHTML = '';
                return;
        ...
        if (scheduleApptForm) scheduleApptForm.addEventListener('submit', function(e) {
            ...
            (async () => {
                ...
                try {
                    const avUrl = 'api/opd/check_doctor_availability.php?doctor_id=' + encodeURIComponent(String(doctorId));
