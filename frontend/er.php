...
<body class="bg-gray-50">
    ...
    <script>
        ...
        async function loadErAssessmentsByPatient(patientId) {
            const res = await fetch(API_BASE_URL + '/er_assessment/list.php?patient_id=' + encodeURIComponent(String(patientId)), { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
        ...
        async function loadErAssessmentsAll(q) {
            ...
            const url = API_BASE_URL + '/er_assessment/list.php?all=1' + (qs ? ('&q=' + encodeURIComponent(qs)) : '');
            const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
                throw new Error((json && json.error) ? json.error : 'Failed to load assessments');
        ...
        if (erAssessForm) {
            erAssessForm.addEventListener('submit', async (e) => {
                ...
                const url = isEditing ? API_BASE_URL + '/er_assessment/update.php' : API_BASE_URL + '/er_assessment/create.php';
        ...
        if (erAssessSendReqSubmitBtn) {
            erAssessSendReqSubmitBtn.addEventListener('click', async () => {
                ...
                try {
                    const res = await fetch(API_BASE_URL + '/er_assessment/submit_to_doctor.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        ...
        async function loadErConsultNotesByPatient(patientId) {
            const res = await fetch(API_BASE_URL + '/er_notes/list.php?patient_id=' + encodeURIComponent(String(patientId)), { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
        ...
        async function erConsultHistoryPatientSearch(q) {
            ...
            const res = await fetch(API_BASE_URL + '/patients/list.php?q=' + encodeURIComponent(text), { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
                resultsEl.classList.remove('hidden');
                resultsEl.innerHTML = '<div class="px-4 py-2 text-sm text-gray-500 bg-white">No result</div>';
        ...
        async function loadErRequests() {
            ...
            const res = await fetch(API_BASE_URL + '/lab/list_requests.php?mode=er', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
                tbody.innerHTML = '';
                return;
        ...
        async function loadErDoctorFeedback() {
            ...
            const res = await fetch(API_BASE_URL + '/er_assessment/feedback_list.php', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) {
        ...
        async function openErDoctorFeedbackView(id) {
            ...
            try {
                const res = await fetch(API_BASE_URL + '/er_assessment/feedback_get.php?id=' + encodeURIComponent(String(id)), { headers: { 'Accept': 'application/json' } });
                const json = await res.json().catch(() => null);
        ...
        if (erConsultSaveBtn) {
            erConsultSaveBtn.addEventListener('click', async () => {
                ...
                const url = isEditing ? API_BASE_URL + '/er_notes/update.php' : API_BASE_URL + '/er_notes/create.php';
