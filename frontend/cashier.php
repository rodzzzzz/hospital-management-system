...
<body class="bg-gray-50">
    <div class="min-h-screen">
        ...
        <main class="ml-16 lg:ml-80 p-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button id="cashierCallNextBtn" onclick="callNextPatient()" class="p-4 bg-blue-600 text-white rounded-lg text-lg font-semibold hover:bg-blue-700 transition-colors flex items-center">
                ...
                <div class="mb-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border-2 border-green-200">
                    ...
                    <div id="cashierCurrentlyServing" class="text-center py-3">
                        <div class="text-gray-500">No patient being served</div>
                    </div>
                    <div id="cashierStationSelection" class="mt-4 hidden flex gap-2 justify-end">
                        ...
                        <button onclick="openCashierSendPatientModal()" class="p-4 bg-green-600 text-white rounded-lg text-lg font-semibold hover:bg-green-700 transition-colors flex items-center">
            ...
            <!-- Send Patient Modal -->
            <div id="cashierSendPatientModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-[60]">
                <div class="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] mx-4 flex flex-col">
                    <div class="p-8 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
                        ...
                        <button type="button" class="text-gray-400 hover:text-gray-600 text-2xl p-2" onclick="closeCashierSendPatientModal()">
                    ...
                    <div class="p-8 flex-1 overflow-y-auto">
                        <div class="mb-6">
                            ...
                            <div id="cashierStationList" class="space-y-4"></div>
                        </div>
                    </div>
                    <div class="p-8 bg-gray-50 border-t flex justify-end gap-4 flex-shrink-0">
                        <button type="button" class="px-8 py-4 border-2 border-gray-200 rounded-lg text-gray-700 hover:bg-gray-100 text-lg font-semibold transition-colors" onclick="closeCashierSendPatientModal()">
                            Cancel
                        </button>
                        <button type="button" id="cashierConfirmSendBtn" class="px-8 py-4 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-lg font-semibold transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed" disabled>
    ...
    <script>
        ...
        async function loadDashboardStats() {
            try {
                const json = await apiGet(API_BASE_URL + '/cashier/stats.php');
        ...
        async function loadRecentInvoices() {
            ...
            if (!tbody) return;
            try {
                const json = await apiGet(API_BASE_URL + '/cashier/list_invoices.php');
        ...
        async function loadCharges(q = '') {
            ...
            if (!tbody) return;
            try {
                const groupUrl = API_BASE_URL + '/cashier/list_charges.php?status=pending_invoice&group=patient' + (q ? ('&q=' + encodeURIComponent(q)) : '');
                const allUrl = API_BASE_URL + '/cashier/list_charges.php?status=pending_invoice' + (q ? ('&q=' + encodeURIComponent(q)) : '');
        ...
        async function openPatientBills(patientId, labChargeId, pharmacyChargeId) {
            try {
                ...
                if (labChargeId) {
                    tasks.push(apiGet(API_BASE_URL + '/cashier/get_charge.php?charge_id=' + encodeURIComponent(String(labChargeId)))
                        .then(j => { out.lab = j.charge || null; }));
                }
                if (pharmacyChargeId) {
                    tasks.push(apiGet(API_BASE_URL + '/cashier/get_charge.php?charge_id=' + encodeURIComponent(String(pharmacyChargeId)))
        ...
        async function loadPayments(q = '') {
            ...
            try {
                ...
                const url = API_BASE_URL + '/cashier/list_invoices.php' + (params.length ? ('?' + params.join('&')) : '');
        ...
        async function openChargeDetails(chargeId) {
            try {
                const json = await apiGet(API_BASE_URL + '/cashier/get_charge.php?charge_id=' + encodeURIComponent(String(chargeId)));
                const c = json.charge || {};
        ...
        async function openInvoiceDetails(invoiceId) {
            try {
                const json = await apiGet(API_BASE_URL + '/cashier/get_invoice.php?invoice_id=' + encodeURIComponent(String(invoiceId)));
                ...
                (async () => {
                    try {
                        ...
                        if (chargeId && chargeId > 0) {
                            await apiPost(API_BASE_URL + '/cashier/pay_charge.php', {
                                charge_id: chargeId,
                                amount: amount,
                                method: (methodRaw || 'cash').toLowerCase().replace(/\s+/g, '_'),
                            });
                        } else {
                            await apiPost(API_BASE_URL + '/cashier/add_payment.php', {
        ...
        async function loadCashierQueue() {
            try {
                const response = await fetch(API_BASE_URL + '/queue/display/4'); // Cashier station ID is 4
        ...
        function updateCashierQueueDisplay() {
                    ...
                    <div class="bg-white p-4 rounded-lg border border-green-300 flex items-center gap-4">
                        <div class="relative h-16 w-16">
                            ...
                            <div class="relative h-full w-full bg-green-500 text-white text-2xl rounded-md flex flex-col items-center justify-center font-bold">
                                ${currentCashierQueueData.currently_serving.queue_number}
                            </div>
                        </div>
                        <div class="flex flex-col items-start text-left">
                            <div class="text-2xl font-bold text-green-700 line-clamp-1">${currentCashierQueueData.currently_serving.full_name}</div>
                            <div class="text-sm text-gray-600">${currentCashierQueueData.currently_serving.patient_code || ''}</div>
                        </div>
                    </div>
                `;
            ...
            if (currentCashierQueueData.next_patients && currentCashierQueueData.next_patients.length > 0) {
                queueListDiv.innerHTML = currentCashierQueueData.next_patients.map((patient) => `
            ...
            if (currentCashierQueueData.unavailable_patients && currentCashierQueueData.unavailable_patients.length > 0) {
                unavailableDiv.innerHTML = currentCashierQueueData.unavailable_patients.map(patient => `
                    <div class="flex justify-between items-center p-2 bg-orange-50 rounded-lg border border-orange-200 cursor-pointer hover:bg-orange-100 transition-colors" onclick="recallCashierUnavailablePatient(${getQueueEntryId(patient)})">
                        <div class="flex items-center space-x-3">
        ...
        function openCashierSendPatientModal() {
            if (!currentCashierQueueData?.currently_serving) {
                Toastify({
            ...
            const modal = document.getElementById('cashierSendPatientModal');
            modal.classList.remove('hidden');
            loadCashierStationsForModal();
        }

        function closeCashierSendPatientModal() {
            const modal = document.getElementById('cashierSendPatientModal');
            modal.classList.add('hidden');
            document.getElementById('cashierConfirmSendBtn').disabled = true;
            document.querySelectorAll('.cashier-station-option').forEach(btn => {
                btn.classList.remove('ring-4', 'ring-blue-500', 'bg-blue-50', 'ring-green-500', 'bg-green-50', 'shadow-lg');
            });
        }

        function loadCashierStationsForModal() {
            const stationList = document.getElementById('cashierStationList');
            ...
            dischargeOption.className = 'cashier-station-option p-6 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-green-400 hover:bg-green-50 transition-all duration-200 transform hover:scale-[1.02]';
            dischargeOption.onclick = () => selectCashierStation('discharge', dischargeOption);
                ...
                .then(data => {
                    ...
                    data.stations.forEach(station => {
                        if (station.id !== 4) {
                            ...
                            stationOption.className = 'cashier-station-option p-6 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-all duration-200 transform hover:scale-[1.02]';
                            stationOption.onclick = () => selectCashierStation(station.id, stationOption);
        ...
        function selectCashierStation(stationId, element) {
            document.querySelectorAll('.cashier-station-option').forEach(btn => {
                btn.classList.remove('ring-4', 'ring-blue-500', 'bg-blue-50', 'ring-green-500', 'bg-green-50', 'shadow-lg');
            });
            ...
            document.getElementById('cashierConfirmSendBtn').disabled = false;
            document.getElementById('cashierConfirmSendBtn').onclick = () => sendCashierPatientToStation(stationId);
        }

        async function sendCashierPatientToStation(stationId) {
            if (!currentCashierQueueData?.currently_serving) return;

            try {
                let body = { queue_id: currentCashierQueueData.currently_serving.id };
                if (stationId !== 'discharge') {
...
<script>window.qrrStationId = 4; window.qrrRefreshQueue = function() { loadCashierQueue(); };</script>
