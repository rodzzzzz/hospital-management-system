...
<script>
    ...
    async function loadEmployees() {
        const res = await fetch(API_BASE_URL + '/hr/employees/list.php?status=active', { headers: { 'Accept': 'application/json' } });
    ...
    async function loadDepartments() {
        const res = await fetch(API_BASE_URL + '/hr/departments/list.php?status=active', { headers: { 'Accept': 'application/json' } });
        const json = await res.json().catch(() => null);
        if (!res.ok || !json || !json.ok) return [];
    ...
    function buildSchedulesUrl() {
        ...
        return API_BASE_URL + '/hr/schedules/list.php' + (qs ? ('?' + qs) : '');
    }
