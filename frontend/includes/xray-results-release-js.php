<script>
    (function () {
        ...
        async function loadXrayResultsRelease() {
            const res = await fetch(API_BASE_URL + '/xray/results_release.php', { headers: { 'Accept': 'application/json' } });
        ...
        async function loadXrayResult(id) {
            ...
            const res = await fetch(API_BASE_URL + '/xray/result.php?' + params.toString(), { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) return null;
            return json;
        }
