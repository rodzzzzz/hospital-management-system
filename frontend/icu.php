...
<body class="bg-gray-50">
    ...
    <script>
        ...
        async function ensureIcuInstalled() {
            try {
                await fetch(API_BASE_URL + '/icu/install.php', { headers: { 'Accept': 'application/json' } });
            } catch (e) {
            }
        }

        async function loadIcuStats() {
            const res = await fetch(API_BASE_URL + '/icu/stats.php', { headers: { 'Accept': 'application/json' } });
        ...
        async function loadIcuAnalytics() {
            const res = await fetch(API_BASE_URL + '/icu/analytics.php', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) return null;
            return json;
        }
        ...
        async function loadIcuPatients() {
            const res = await fetch(API_BASE_URL + '/icu/patients.php', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) return null;
        ...
        async function loadIcuLabs() {
            const res = await fetch(API_BASE_URL + '/icu/labs.php', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) return null;
        ...
        async function loadIcuTransfers() {
            const res = await fetch(API_BASE_URL + '/icu/transfers.php', { headers: { 'Accept': 'application/json' } });
            const json = await res.json().catch(() => null);
            if (!res.ok || !json || !json.ok) return null;
